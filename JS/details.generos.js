...
.then(function(data){
    ...
    if (data.results.length == 0) {
        ...
        contenedor.innerHTML += ` <h2 style="color:white;">No hay mas peliculas o series del genero: ${id}</h2> `
    }
    else {
        contenedor.innerHTML += ` <h2 style="color:white;">Mas peliculas y series del genero: ${id}</h2> `}
...
let generosSeries = document.querySelector('.generosSerie')
    generosSeries.innerHTML += ` <h2 style="color:white;">Mas series del genero: ${id}</h2> `
    for (let i=0; i < 5 ; i++){
